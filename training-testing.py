...
from model import RNN #importing my model from model.py file
...
n_features = 11 #the number of features
...
df_train = scaler.transform(df_train) #returns a numpy array

df_test = df.drop("date_time",1)[5000:] 
df_test = scaler.transform(df_test)

#RESHAPING the problem from TIME SERIES to SUPERVISED LEARNING
#X shape --> (length-sequence_length),sequence_length,n_features = 4900,100,11; one X datapoint shape --> sequence_length,n_features = 100,11
#y shape --> (length-sequence_length),targets = 4900,11; one y labels shape --> targets = 3
class Train_Time_Series_Dataset(Dataset):
    ...
    def __init__(self, complete_data,sequence_length):
        ...
        for i in range(len(complete_data)):
            ...
            #seq_x shape --> sequence_length, n_features (for time steps up to t-1)
...
#Eval_Dataset is basically used mainly to convert a numpy arrays to tensors and return the data of shape --> "len of input data frame",n_features
...
train_loader = DataLoader(dataset=dataset,batch_size=batch_size,shuffle=True) #train_loader is used to supply data in randomly shuffled BATCHES during training
...
#MODEL    
model = RNN(n_features,n_layers,hidden_size,output_size,sequence_length).to(device)
...
for epoch in range(num_epochs):
    for i, (features,targets) in enumerate(train_loader):
        #features shape --> batch_size, sequence_length, n_features
...
test_outputs = model.predict(test_dataset[100:600],test_dataset[600:800]) #getting the outputs for data in test_dataset in the second argument
